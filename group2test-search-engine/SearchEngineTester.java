...
public class SearchEngineTester {
	...
	freq=Bytes.toInt(value); 
	Result dRow=dataTable.get(new Get(kv.getQualifier())); 
	byte[] contentBytes = dRow.getValue(Bytes.toBytes("details"), Bytes.toBytes("URI")); 
	pageUri = Bytes.toString(contentBytes); 
	Result dVal=prTable.get(new Get(kv.getQualifier())); 
	//byte []rank=dVal.value();
        for (KeyValue contentRow : dVal.list()){
            if (contentRow.matchingFamily(Bytes.toBytes("pagerank"))){
                pageRank = Bytes.toFloat(contentRow.getQualifier());
				break;
            }
        }
	//pageRank=Bytes.toFloat(rank);
	//pageRank=Bytes(tmp);
